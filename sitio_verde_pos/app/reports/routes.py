...
def _pax_breakdown(start, end):
    """Guest-type breakdown (Adult/Senior/PWD/Kids/PWD Kids/Free) for buffet
    sale items within the given date range, completed sales only."""
    row = (
        db.session.query(
            ...
            func.coalesce(func.sum(SaleItem.buffet_pwd_kids), 0),
    ...
    adult, senior, pwd, kids, pwd_kids, free = row if row else (0, 0, 0, 0, 0, 0)
    total = adult + senior + pwd + kids + pwd_kids + free

    def pct(n):
        return round((n / total) * 100, 1) if total else 0.0

    return {
        "adult": adult, "senior": senior, "pwd": pwd, "kids": kids, "pwd_kids": pwd_kids, "free": free,
        "total": total,
        "pct": {
            "adult": pct(adult), "senior": pct(senior), "pwd": pct(pwd),
            "kids": pct(kids), "pwd_kids": pct(pwd_kids), "free": pct(free),
        },
    }
